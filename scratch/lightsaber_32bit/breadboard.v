// ============================================================
// Module: breadboard
// Project: Lightsaber Control Circuit (32-bit)
// Cohort:  Halo 2 (2004)
...
//   0010 - Set blade length     A (1010) - Toggle Power
//   0011 - Increment length     B (1011) - Double Length
//   0100 - Decrement length     C-F - Reserved (invalid)
...
//   blade_color  - 32-bit current blade color register
//   blade_count  - 32-bit current blade count register
//   error_reg    - 32-bit accumulated error status register
...
wire [31:0] inc_length;   // blade_length + 1 (from adder)
wire [31:0] dec_length;   // blade_length - 1 (from subtractor)
wire [31:0] dbl_length;   // blade_length * 2 (from adder)
wire        inc_cout;     // carry out of increment (overflow flag)
wire        dec_bout;     // borrow out of decrement (underflow flag)
wire        dbl_cout;     // carry out of double (overflow flag)

// MUX chain output interfaces (next register values)
wire [31:0] next_length;
wire [31:0] next_color;
...
wire [31:0] len_mux_set;  // after Set Length MUX
wire [31:0] len_mux_inc;  // after Increment MUX
wire [31:0] len_mux_dec;  // after Decrement MUX

// Intermediate MUX chain wires — power and lock
wire pwr_mux_off;        // after Power OFF MUX
wire pwr_mux_on;         // after Power ON MUX
...
wire err_dbl_overflow;      // error[3]: double while would overflow
wire err_invalid_opcode;    // error[4]: opcode >= 1100
wire err_power_off_cmd;     // error[0]: command while powered off
wire err_locked_cmd;        // error[1]: op command while locked

wire [31:0] computed_error;  // newly detected errors this cycle
wire [31:0] error_accum;     // current errors OR new errors
...
wire any_op_cmd;            // decode[2..11]: any operation opcode
wire any_blocked_op;        // decode[2..6]: ops blocked by lock
wire inv_op_c, inv_op_d;    // intermediate OR for invalid opcode
...
// Opcode 1010 (toggle power): always active (doesn't require power to toggle)
assign active_dec[10] = decode[10];

// Opcode 1011 (double length): requires power ON and NOT locked
and g_gate11 (active_dec[11], decode[11], power_status, not_lock_status); // Double Length

// Reserved opcodes 1100–1111: pass through for error detection
assign active_dec[15:12] = decode[15:12];

// ============================================================
// COMPONENT 3: 32-BIT ADDER — blade length increment
// Computes blade_length + 1 each cycle (result used if op=0011)
// ============================================================
adder32 length_incrementer (
    .a   (blade_length),
    .b   (32'd1),
    .cin (1'b0),
    .sum (inc_length),
...
// ============================================================
// COMPONENT 3B: 32-BIT ADDER — blade length double
// Computes blade_length + blade_length (x2) each cycle (result used if op=1011)
// ============================================================
adder32 length_doubler (
    .a   (blade_length),
    .b   (blade_length),
    .cin (1'b0),
    .sum (dbl_length),
...
// ============================================================
// COMPONENT 4: 32-BIT SUBTRACTOR — blade length decrement
// Computes blade_length - 1 each cycle (result used if op=0100)
// ============================================================
subtractor32 length_decrementer (
    .a   (blade_length),
    .b   (32'd1),
    .diff(dec_length),
    .bout(dec_bout)
...
//   Hold (default) → Set (0010) → Increment (0011) → Decrement (0100) → Double (1011)
// Since opcodes are mutually exclusive (one-hot decoder),
// only one MUX stage fires per cycle.
// ============================================================
mux2to1_32bit len_mux_set_inst (
...
mux2to1_32bit len_mux_inc_inst (
...
mux2to1_32bit len_mux_dec_inst (
...
mux2to1_32bit len_mux_dbl_inst (
    .sel(active_dec[11]), // opcode 1011: Double Length
    .a  (len_mux_dec),    // previous MUX result
    .b  (dbl_length),     // use doubled value if selected
    .out(next_length)
);
...
// Hold → Power OFF (0000) → Power ON (0001) → Toggle Power (1010)
// ============================================================
...
mux2to1_1bit pwr_toggle_mux (
    .sel(active_dec[10]), // opcode 1010: Toggle Power
    .a  (pwr_mux_on),
    .b  (not_power_status), // toggle: set to ~power_status
    .out(next_power)
);
...
//   error_reg[4] — Invalid opcode (1100–1111)
//   error_reg[31:5] — Reserved (always 0)
// ============================================================

// Error[3]: Overflow — increment active AND carry out is high
and g_err_ovf  (err_length_overflow,  active_dec[3], inc_cout);

// Error[3]: Overflow from double — double active AND carry out is high
and g_err_dbl_ovf (err_dbl_overflow, active_dec[11], dbl_cout);

// Error[2]: Underflow — decrement active AND borrow out is high
and g_err_udf  (err_length_underflow, active_dec[4], dec_bout);

// Error[4]: Invalid opcode — any reserved opcode (1100–1111)
or  g_err_inv_c (inv_op_c, decode[12], decode[13]);
or  g_err_inv_d (inv_op_d, decode[14], decode[15]);
or  g_err_inv   (err_invalid_opcode, inv_op_c, inv_op_d);

// Error[0]: Command while powered off
// Triggered by opcodes 0010-0110, 1011 while powered off.
// Power ON (0001), Power OFF (0000), and Toggle Power (1010) are exempt — always allowed.
or  g_any_op (any_op_cmd, decode[2], decode[3],
              decode[4], decode[5], decode[6], decode[7],
              decode[8], decode[9], decode[11], inv_op_c, inv_op_d);
and g_err_pwr (err_power_off_cmd, any_op_cmd, not_power_status);

// Error[1]: Operation while locked
// Triggered if an operation opcode (0010–0110, 1011) arrives while locked
or  g_blocked_op (any_blocked_op, decode[2], decode[3],
                  decode[4], decode[5], decode[6], decode[11]);
and g_err_lck (err_locked_cmd, any_blocked_op, lock_status);

// Combine overflow errors: either increment overflow OR double overflow
wire combined_overflow;
or g_combined_ovf (combined_overflow, err_length_overflow, err_dbl_overflow);
...
assign computed_error[3] = combined_overflow;
...
// ============================================================
register32 len_reg  (.clk(clk), .reset(reset), .d(next_length), .q(blade_length));
register32 col_reg  (.clk(clk), .reset(reset), .d(next_color),  .q(blade_color));
register32 cnt_reg  (.clk(clk), .reset(reset), .d(next_count),  .q(blade_count));
register32 err_reg  (.clk(clk), .reset(reset), .d(next_error),  .q(error_reg));
